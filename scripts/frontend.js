(() => {
	...
	window.wpz_payments_format_price = (amount, noCurrency) => {
		if (isNaN(amount)) {
			amount = 0;
		}
	...
	window.wpz_payments_pay_button_text = ($container, textTemplate) => {
	...
	window.wpz_payments_pay_button_is_disabled = ($container) => {
	...
	window.wpz_payments_handle_keydown = (ev) => {
	...
	window.jQuery(document.body).on('click', '.wpz-payments-type-options a', function() {
		...
		$container
			.removeClass('wpz-payments-stripe wpz-payments-paypal')
			.addClass('wpz-payments-' + changeTo);
	...
	window.wpz_payments_init = (containerElem) => {
		var instance = {
			id: nextInstanceId++,
		...
		if (instance.type === 'stripe') {
			var updateStripePaymentIntent = (cb) => {
				globalSettings.stripe.controllingInstance = null;
				
				var $button = instance.$container.find('.wpz-payments-pay-button:first');
				
				var onUpdateError = () => {
					$button.removeClass('ds-dpm-loading').attr(
						'disabled',
						wpz_payments_pay_button_is_disabled(instance.$container)
					);
				};
				
				$button.text(
					wpz_payments_pay_button_text(instance.$container, $button.attr('data-text-template'))
				)
				.addClass('ds-dpm-loading')
				.attr(
					'disabled',
					true
				);
				...
				globalSettings.stripe.api.retrievePaymentIntent(globalSettings.stripe.intent)
				.then( (response) => {
					...
					window.jQuery.post(
						window.WpzPaymentsFrontendData.ajax_url,
						preflightData,
						function(response) {
							if (response.success) {
								...
								if (response.data && response.data.newIntentId) {
									...
									if (message) {
										window.jQuery('<div>')
											.addClass('wpz-payments-reload-info')
											.text( message )
											.insertBefore(instance.$container.find('.wpz-payments-form:first'));
									}
								}
							}
						},
		...
		var onError = (reason, noReload) => {
			...
			window.jQuery('<div>').addClass('wpz-payments-error').text(
				errorMessageTemplate.replaceAll(/%\{error_reason\}/g, reason ? reason : window.wp.i18n.__('Unknown', 'wpz-payments'))
			).appendTo( $modalContent.length ? $modalContent : instance.$container );
		...
		var onSuccess = () => {
			...
			var $modalContent = instance.$container.find('.wpz-payment-modal-content:first');
			window.jQuery('<div>').addClass('wpz-payments-success').text(
		...
		switch (instance.type) {
			case 'stripe':
				instance.$container.on('click', '.wpz-payments-pay-button', function() {
					...
					if ($button.attr('disabled') || isVisualBuilder) {
					} else {
						...
						if ($agreementCheckbox.length) {
							$agreementCheckbox.parent().parent().before(
								window.jQuery('<p>').addClass('wpz-payments-validation-error wpz-payments-agreement-validation-error').text( $agreementCheckbox.attr('data-validation-message') )
							);
							return;
						}
						...
						if (globalSettings.stripe.controllingInstance !== instance.id) {
							updateStripePaymentIntent( () => {
								window.jQuery('.wpz-payments-confirm').remove();
								window.jQuery('<div>')
									.addClass('wpz-payments-confirm')
									.append(
										window.jQuery('<p>').text( wpz_payments_pay_button_text(instance.$container, instance.$container.attr('data-confirm-message')) )
									)
									.append(
										window.jQuery('<div>').addClass('wpz-payments-button-confirm-wrapper')
											.append(
												window.jQuery('<button>').attr('type', 'button').addClass('wpz-payments-confirm-yes wpz-payments-pay-button et_pb_button').text( instance.$container.attr('data-confirm-yes') )
											)
											.append(
												window.jQuery('<button>').attr('type', 'button').addClass('wpz-payments-confirm-no et_pb_button wpz-payments-button-secondary').text( instance.$container.attr('data-confirm-no') ).on('click', (ev) => { window.jQuery(ev.target).closest('.wpz-payments-confirm').remove() })
											)
									)
			...
			case 'paypal':
				...
				var setupPayment = (data, pp) => {
					...
					// PayPal doesn't support decimal quantities
					while (quantity % 1) {
						quantity *= 10;
						price /= 10;
					}
				...
				instance.paypalOptions = {
					...
					onApprove: doPayment,
					onCancel: () => onComplete(),
					onError: () => { if (!instance.$container.has('.wpz-payments-agreement-validation-error').length) { onError(); } }
				};
	...
	var arrayDeepCompare = (arr1, arr2) => {
	...
	window.wpz_payments_set_styles = (instanceId, styles, stripeTheme) => {
		
		if (!instances['i' + instanceId] || instances['i' + instanceId].type !== 'stripe') {
			return;
		}
		
		var neededComputedProps = [
			'fontFamily',
			'fontSize',
			'lineHeight',
			'color',
		...
		var computedStyles = [ // 0 = text input, 1 = label, 2 = p
			...
			$item = window.jQuery(item)
				.prependTo(instances['i' + instanceId].$container);
				...
				var defaultStyles = {
					999999: [
						[
							['.Input'],
							[
								['fontFamily', computedStyles[0].fontFamily],
								['fontSize', '14px'],
								['lineHeight', 'normal'],
								['color', '#999'],
								['padding', '16px'],
						...
						[
							['.Label'],
							[
								['fontFamily', computedStyles[1].fontFamily],
								['fontSize', computedStyles[1].fontSize],
								['lineHeight', computedStyles[1].lineHeight],
								['color', computedStyles[1].color],
								['marginBottom', '0.5em'],
								['fontWeight', computedStyles[1].fontWeight]
							]
						],
						[
							['.Error'],
							[
								['fontFamily', computedStyles[2].fontFamily],
								['fontSize', computedStyles[2].fontSize],
								['lineHeight', computedStyles[2].lineHeight],
								['color', 'red'],
								['fontWeight', computedStyles[2].fontWeight]
							]
						]
					]
				};
		...
		for (var mq in stylesByMq) {
			stylesByMq[mq] = stylesByMq[mq].split('}').map( (block) => {
				return [
					block.substring( 0, block.indexOf('{') ).trim(),
					block.substring( block.indexOf('{') + 1 ).trim()
				];
			} ).filter( (block) => {
				...
				return [
					block[0]
						...
						.filter( (selector) => (selector.indexOf(' #STRIPE ') !== -1 || selector.indexOf(' #STRIPE.theme-' + stripeTheme + ' ') !== -1) )
						.map( (selector) => selector.substring(selector.indexOf(' ', selector.indexOf(' #STRIPE') + 8)).trim() ),
					block[1]
						.split(';')
						.filter( (rule) => rule.length )
						.map( (rule) => {
									rule = rule.split(':').map( (rulePart) => rulePart.trim() );
									rule[0] = rule[0]
												.split('-')
												.map( (word, index) => index ? word[0].toUpperCase() + word.substring(1).toLowerCase() : word.toLowerCase() )
												.join('');
			...
			switch (instance.type) {
				case 'stripe':
					...
					var initForm = () => {
						...
						if (!globalSettings.stripe || !globalSettings.stripe.api || !globalSettings.stripe.intent || !globalSettings.stripe.publicKey || globalSettings.stripe.hasOwnProperty('initErrorMessage')) {
							...
							window.jQuery('<div>').addClass('wpz-payments-error').text(
								globalSettings.stripe.initErrorMessage
									? wp.i18n.__('An error occurred while loading the payment form: %s', 'wpz-payments').replace('%s', globalSettings.stripe.initErrorMessage ? globalSettings.stripe.initErrorMessage:  __('Unspecified error', 'wpz-payments'))
									:  instance.$container.attr('data-load-error-message')
							).prependTo($modalContent.length ? $modalContent : instance.$container);
							return;
						}
						...
						for ( var selector in appearance.rules ) {
							if (appearance.rules[selector].fontFamily) {
								fonts = fonts.concat(
									appearance.rules[selector].fontFamily
										.split(',')
										.map( (fontFamily) => {
										...
										.filter( (fontFamily) => fonts.indexOf(fontFamily) === -1 )
								);
							}
						}
						...
						fonts = fonts.map( (font) => {
							...
							if ($font.length) {
								...
								var fontLinks = window.jQuery('link[href*="' + fontUrlSearch + '"]').filter( (i, elem) => {
									var hrefIndex = elem.href.indexOf(fontUrlSearch), hrefIndexAfter = hrefIndex + fontUrlSearch.length;
							...
							return fontUrl ? { cssSrc: fontUrl } : {};
						} ).filter( (font) => font.cssSrc );
						...
						paymentElement.on( 'ready', () => { instance.$container.removeClass('wpz-payments-loading'); instance.$container.find('.wpz-payments-pay-button').removeClass('wpz-payments-disabled').attr('disabled', false); } );
				...
				case 'paypal':
					if (!window.paypal || !window.paypal.Buttons) {
						...
						instance.paypalButtons
							.render( $buttonContainer[0] )
							.then( () => { instance.$container.removeClass('wpz-payments-loading'); } );
